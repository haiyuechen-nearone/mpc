...
9. Each $P_i$ computes its signature share $s_i \gets \alpha_i \cdot h + \beta_i \cdot R_\mathsf{x} + e_i$ where $R_\mathsf{x}$ is the x coordinate of $R$.
...
16. $\blacktriangle$ The coordinator asserts that $(R, s)$ is a valid ECDSA signature for $h$ under $Y$.
